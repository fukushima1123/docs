...
この原則に違反しているコードのことを福島は未来予知コードと呼んでいます。
...
例えばあるボタンのcomponentの引数に `handleCreate` があると仮定しましょう。下の定義の方がより優れています。
...
関数を定義するとき、以下のようにブロックではなく、`=`で定義できないかを考えよう。
...
Bill Oneにおいてはプロダクションコードでは `let` と `also` だけ使っていればユースケースの99%をカバーできることを抑えておきましょう。
...
テストのみの使用を強く推奨します。
スコープが広がりすぎるため読み手の思考負荷を増強させる為です。
...
null使うな（意味論）
謎のgetter生やすな（責務違反）
copy()の制限(private constructorの話を書くか・・・？)

#### Aggregate

- ListではなくSetを使おう
- ValidationはValidationObjectを作るかcompanion objectにするか
- First Class Collectionとか無意味だからやめてね
- 構成するValue Object同士の整合を制約で
- methodは全てResultを返す
  - modifyするときはproperty一つずつ
